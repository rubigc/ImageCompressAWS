...
aws lambda create-function --function-name "$LAMBDA_FUNCTION_NAME" --zip-file fileb://function.zip --handler index.andler --runtime nodejs16.x --role $LABROLE
...
aws s3 cp s3://$BUCKET_NAME_SRC/$OBJEKT_SCHLUESSEL $LOKALER_ORDNER --region $REGION
